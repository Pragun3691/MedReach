...
function NotificationEntry({ userId, inverse = false, editorial = false }) {
...
export function PublicHeader({ overlay = false, editorial = false }) {
    ...
    <header className={`${overlay ? 'fixed inset-x-0 top-0 z-50' : editorial ? 'sticky top-0 z-50' : 'relative'} border-b transition-[background-color,border-color,box-shadow,backdrop-filter] duration-300 ${inverse ? 'border-white/15 bg-transparent' : editorial ? 'border-[#0F2747]/12 bg-[#F8F6F1]/96 shadow-[0_10px_30px_-25px_rgba(15,39,71,0.55)] backdrop-blur-md' : 'border-slate-200/90 bg-[#F8F6F1]/95 shadow-[0_10px_30px_-24px_rgba(15,39,71,0.7)] backdrop-blur-md'}`}>
      <div className={`mx-auto flex max-w-7xl items-center justify-between gap-3 px-5 transition-[min-height] duration-300 sm:px-8 lg:px-10 ${hasScrolled ? 'min-h-16' : 'min-h-[4.75rem]'}`}>
        ...
        <nav className="flex items-center gap-3 sm:gap-5 lg:gap-8" aria-label="Primary navigation">
          <NavLink className={({ isActive }) => `hidden md:block ${navigationClass({ isActive }, inverse, editorial)}`} to="/doctors">Find Doctors</NavLink>
          ...
          {status === 'anonymous' && (
            <>
              <Link className={`header-link text-sm font-medium ${inverse ? 'text-white/80 hover:text-white' : editorial ? 'text-[#43545F] hover:text-[#173960]' : 'text-slate-700 hover:text-[#173960]'}`} to="/login">Login</Link>
      ...
      {status === 'authenticated' && currentUser && (
        <nav className={`mx-auto flex max-w-7xl gap-5 border-t px-5 py-2.5 md:hidden ${inverse ? 'border-white/15' : 'border-slate-100'}`} aria-label="Account navigation">
          <NavLink className={({ isActive }) => navigationClass({ isActive }, inverse, editorial)} to="/doctors">Find Doctors</NavLink>
