  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/45 sm:items-center sm:p-6" onMouseDown={event => event.target === event.currentTarget && guardedClose()} role="presentation">
      <section className="w-full rounded-t-2xl bg-white p-6 shadow-2xl sm:max-w-lg sm:rounded-2xl" ref={dialogRef} role="dialog" aria-labelledby="cancel-appointment-heading" aria-modal="true">
        <p className="text-sm font-semibold text-rose-700">{doctorRequired ? 'Cancel appointment' : t('cancellation.eyebrow')}</p>
        <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950" id="cancel-appointment-heading">{doctorRequired ? 'Are you sure?' : t('cancellation.sure')}</h2>
        <p className="mt-3 leading-7 text-slate-600">
          {doctorRequired
            ? 'This slot will be removed from availability and the Patient will be notified.'
            : t('cancellation.patientCopy')}
        </p>

        <form className="mt-5" onSubmit={submit}>
          <label className="block text-sm font-semibold text-slate-800" htmlFor="cancellation-reason">
            {doctorRequired ? 'Reason (required)' : `${t('cancellation.reason')} ${t('cancellation.optional')}`}
          </label>
          <textarea
            ...
            placeholder={doctorRequired ? 'Explain why the appointment cannot go ahead' : t('cancellation.placeholder')}
            value={reason}
          />
          {error && <p className="mt-3 rounded-lg bg-rose-50 p-3 text-sm text-rose-800" role="alert">{error}</p>}
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <button className="min-h-11 rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50" disabled={pending} onClick={guardedClose} ref={cancelButtonRef} type="button">{doctorRequired ? 'Keep appointment' : t('cancellation.keep')}</button>
            <button className="min-h-11 rounded-lg bg-rose-700 px-4 text-sm font-semibold text-white hover:bg-rose-800 disabled:cursor-wait disabled:opacity-60" disabled={pending} type="submit">{pending ? (doctorRequired ? 'Cancelling…' : t('cancellation.cancelling')) : (doctorRequired ? 'Cancel appointment' : t('cancellation.cancel'))}</button>
          </div>
        </form>
      </section>
    </div>
